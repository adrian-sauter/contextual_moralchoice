...
parser.add_argument(
    "--target-dir",
    default="openai_batches",
    type=str,
    help="Base path for output folder (will be suffixed with model name and part number)",
)
parser.add_argument(
    "--reasoning-effort",
    default="none",
    type=str,
    help="Reasoning Effort for GPT-5.1 (none (default), low, high)",
)
...
base_path = f"data/scenarios/{args.target_dir}"
os.makedirs(base_path, exist_ok=True)

for i in range(num_batches):
    start = i * BATCH_LIMIT
    end = min((i + 1) * BATCH_LIMIT, len(batch_lines))
    split_path = f"{base_path}/{args.model_name}_part{i+1}.jsonl"
